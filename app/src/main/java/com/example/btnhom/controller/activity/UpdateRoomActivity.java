...
public class UpdateRoomActivity extends AppCompatActivity {
    ...
    private TextInputEditText tietRoomId, tietRoomName, tietRentPrice, tietArea, tietTenantName, tietPhoneNumber, tietNotes;
    ...
    private void loadRoomData() {
        ...
        if (roomId != null) {
            ...
            if (currentRoom != null) {
                tietRoomId.setText(currentRoom.getRoomId());
                tietRoomName.setText(currentRoom.getRoomName());
                tietRentPrice.setText(String.valueOf(currentRoom.getRentPrice()));
                tietArea.setText(String.valueOf(currentRoom.getArea()));
                tietNotes.setText(currentRoom.getNotes());
    ...
    private void saveRoom() {
        ...
        String areaStr = tietArea.getText().toString().trim();
        String notes = tietNotes.getText().toString().trim();
        ...
        try {
            ...
            tietArea.setError(getString(R.string.error_invalid_area));
            return;
        }

        // Cập nhật đối tượng currentRoom
        currentRoom.setRoomName(name);
        currentRoom.setRentPrice(price);
        currentRoom.setArea(area);
        currentRoom.setNotes(notes);
